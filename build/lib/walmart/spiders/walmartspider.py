...
def get_three_max_values(count_values):
    max_values = []
    max1 = 0
...
class WalmartspiderSpider(scrapy.Spider):
    ...
    def parse_method(self, response):
        ...
        # req = self.set_proxies("https://www.walmart.com/ip/Ozark-Trail-4-Season-7-8-x-4-8-x-3-8-Hiker-Tent-Sleeps-2/46425207", self.parse_item_detail_page)
        ...
        if self.method == "":
            ...
            logging.info("------------------- Retrieve Product List From Database : Start --------------------")
            self.product_list =  db.retrieve_data("Select product_id From " + self.category_table)
            logging.info("------------------- Total : " + str(len(self.product_list)) + " --------------------")
            logging.info("------------------- Retrieve Product List From Database : End --------------------")
            # category_divs = response.xpath("//li[@class='department-single-level']/a")
            ...
            for url in self.category_urls:
                req = self.set_proxies(response.urljoin(url) , self.prepare_search_condition)
                req.meta["url"] = url
    ...
    def prepare_search_condition(self, response):
        search_urls = []
        search_urls.append(response.meta["url"])

        category_url = response.meta["url"].replace("https://www.walmart.com/browse?", "")
        ...
        for script in script_lists:
            ...
            if content != "":
                ...
                total_count = json_obj["preso"]["requestContext"]["itemCount"]["total"]
                
                facets = json_obj["preso"]["facets"]
                counts = []
                for facet in facets:
                    if facet["name"] != "Departments":
                        counts.append(len(facet["values"]))
                
                max_cnts = get_three_max_values(counts)
                ...
                for facet in facets:
                    if facet["name"] != "Departments":
                        for max_cnt in max_cnts:
                            if len(facet["values"]) == max_cnt:
                                search_dicts[facet["name"]] = facet
                
                
                search_conditions = []
                for key in search_dicts.keys():
                    exist = 0
                    for value in search_dicts[key]["values"]:
                        if "&" in value["url"]:    
                            exist = 1
                    
                    if exist == 1:
                        search_conditions.append(search_dicts[key])
                
                # for search1 in search_conditions:
                #     logging.info(search1["name"])

                for facet in facets:
                    if facet["name"] == "Price":
                        for value in facet["values"]:
                            search_cond = value["url"].replace(category_url + "&", "")
                            url = response.meta["url"] + "&" + search_cond + "&sort=new"
                            search_urls.append(url)
                
                for search1 in search_conditions[0]["values"]:
                    search1_cond = search1["url"].replace(category_url + "&", "")

                    if total_count > 10000:
                        second_search_url_cnt = 0
                        for search2 in search_conditions[1]["values"]:
                            search2_cond = search2["url"].replace(category_url + "&", "").replace("facet=", "")

                            if total_count > 600000:
                                search_url_cnt = 0
                                for search3 in search_conditions[2]["values"]:
                                    search3_cond = search3["url"].replace(category_url + "&", "").replace("facet=", "")

                                    search_url_cnt += 1

                                    if search_url_cnt > (total_count / 500 / len(search_conditions[0]["values"]) / len(search_conditions[1]["values"])):
                                        continue

                                    url = response.meta["url"] + "&" + search1_cond + "||" + search2_cond + "||" + search3_cond + "&sort=new"
                                    search_urls.append(url)


                            else:
                                second_search_url_cnt += 1

                                if second_search_url_cnt > (total_count / 500 / len(search_conditions[0]["values"])):
                                    continue

                                url = response.meta["url"] + "&" + search1_cond + "||" + search2_cond + "&sort=new"
                                search_urls.append(url)
                    else:
                        url = response.meta["url"] + "&" + search1_cond + "&sort=new"
                        search_urls.append(url)

                logging.info("************Category-> " + category_url + " ----->" + str(total_count) + " --> Search Urls :" + str(len(search_urls)))
                
                for url in  search_urls:
                    req = self.set_proxies(url , self.parse_category_items)
                    req.meta["url"] = response.meta["url"]
                    req.meta["current_time"] = response.meta["current_time"]    
                    req.meta["index_count"] = 0
                    req.meta["category_url"] = url
                    sleep(0.05)
                    yield req
    ...
    def parse_item_detail_page(self, response):
        ...
        for script in script_lists:
            ...
            if content != "":
                ...
                try:
                    product_id =  json_prod["primaryProduct"].encode('utf-8').strip()
                except:
                ...
                try:
                    ...
                    logging.info("*** Project Not Found **** " + "," + response.url)
                    return
                ...
                if response.meta["type"] == "upload":
                    ...
                    if ids_exists == 0:
                        logging.info("___________Upload Scraiping ID Doesn't Exists___________")
                        return

                #Product Category Path
                try:
                    product["category"] = product_obj["productAttributes"]["productCategory"]["categoryPath"].encode('utf-8').strip()
                except:
                    for key in json_prod["products"].keys():
                        temp_product = json_prod["products"][key]

                        if product["category"] == "":
                            try:
                                product["category"] = temp_product["productAttributes"]["productCategory"]["categoryPath"].encode('utf-8').strip()
                            except:
                                product["category"] = ""
                ...
                try:
                    ...
                    for key1 in json_prod["idmlMap"].keys():
                        temp_product = json_prod["idmlMap"][key1]

                        if len(product["details"]) == 0:
                            try:
                                specifications =  temp_product["modules"]["Specifications"]["specifications"]["values"][0]
                                for specification in specifications:
                                    for key in specification.keys():
                                        sp_item = {}
                                        sp_name = specification[key]["displayName"]
                        ...
                        for sub_item in sub_item_list:
                            for key in option_product_list.keys():
                                if key == sub_item["productId"]:
                                        ...
                                        #logging.info("Offer Size is more than 1 **** " + str(product["product_id"]) +"," +response.url)
        ...
        if product["pictures_url"] != "":
            ...
            logging.info("-----------Product Doesn't have picture---------" + str(product["product_id"]) + "  -----   " + response.url)
    ...
    def parse_category_items(self, response):
            ...
            for script in script_lists:
                ...
                if content != "":
                    ...
                    for item in items:
                        ...
                        product["usItemId"] = item["usItemId"].encode('utf-8').strip()
                        product["product_id"] = item["productId"].encode('utf-8').strip()

                        exist = 0
                        for db_item in self.product_list:
                            if db_item[0] ==  product["product_id"]:
                                #logging.info("*****Product ID Exists in Database*******" + product["product_id"] +  response.url)    
                                exist = 1
                        ...
                        try:
                            ...
                            logging.info("*********Product Type Error**********" + str(product["product_id"])+ ","+ response.url)
                            continue

                        if product["product_type"]  == "BUNDLE":
                            logging.info("------- Product is Bundle --------" + str(product["product_id"])+ ","+ response.url)
                            continue
                        ...
                        try:
                            ...
                            logging.info("********Product URL Error********" + str(product["product_id"])+ ","+ response.url)
                            continue
                        ...
                        try:
                            ...
                            logging.info("********Tow Days Shipping Error********" + str(product["product_id"])+ ","+ response.url)
                            product["delivery_time"] = ""
                        ...
                        try:
                            ...
                            logging.info("*******Seller Error*******" + str(product["product_id"])+ ","+ response.url)
                            continue
                        ...
                        try:
                            ...
                            logging.info("*********Offer Price Error*******" + str(product["product_id"])+ ","+ response.url)
                            continue
                        ...
                        for p in self.item_list:
                            if p["product_id"] == product["product_id"]:
                                #logging.info("******Product Exist*****" + str(p["product_id"]) + "," + response.url)
                                exists = True

                        if exist == False:
                            response.meta["index_count"] += 1
                    ...
                    #logging.info("-----------> Total->" +  response.meta["category_url"].replace("https://www.walmart.com/browse?cat_id=5431_1095032", "") + "," +str(total_count) + "," + str(response.meta["index_count"]))
        ...
        if next_page_url != "":
            ...
            req.meta["url"] = response.meta["url"]
            req.meta["current_time"] = response.meta["current_time"]
            req.meta["index_count"] = response.meta["index_count"]
            req.meta["category_url"] = response.meta["category_url"]
            yield req
            ...
            for script in script_lists:
                ...
                if content != "":
                    ...
                    for item in items:
                        ...
                        try:
                            product["product_url"] = response.urljoin(item["productPageUrl"]).encode('utf-8').strip()
                        except:
